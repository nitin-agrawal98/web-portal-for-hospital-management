...
export class AppointmentsHistoryComponent implements OnInit {
  ...
  getAppointments(): void {
    if (this.isPatient) {
      this.service.getHistoryAppointmentsByPatientWithFilters(this.pageNumber, this.recordsPerPage, this.filterBy, this.filterValue, this.healthProblem)
        .subscribe(
          pageDetails => {
      ...
      this.service.getHistoryAppointmentsByDoctorWithFilters(this.pageNumber, this.recordsPerPage, this.filterBy, this.filterValue, this.healthProblem)
        .subscribe(
          pageDetails => {
            if (pageDetails.content.length > 0) {
              this.handle(pageDetails);
      ...
      this.service.getHistoryAppointmentsWithFilters(this.pageNumber, this.recordsPerPage, this.filterBy, this.filterValue, this.healthProblem)
        .subscribe(
          pageDetails => {
            if (pageDetails.content.length > 0) {
              this.handle(pageDetails);
  ...
  handle(pageDetails): void {
    ...
    this.isLastPage = this.pageNumber === pageDetails.totalPages - 1;
